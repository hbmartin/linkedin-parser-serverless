...
interface CanonicalHeaderCandidateStartParams {
  index: number;
  lineTexts: string[];
}
...
interface WrappedDescriptionContinuationFragmentParams {
  allLines: NormalizedParserLine[];
  index: number;
  line: NormalizedParserLine;
}
...
export class ExperienceStructuralParser {
  ...
  private static shouldMergeWrappedPositionTitle({
    ...
    return (
      ...
      (this.WRAPPED_TITLE_KEYWORD_PATTERN.test(line.text) ||
        ...
        hasWrappedBusinessTitle) &&
      (this.looksLikePendingTitleContinuationLine(nextLine.text) ||
        (hasWrappedBusinessTitle &&
          this.looksLikeTerminalTitleContinuationLine(nextLine.text))) &&
  ...
  private static shouldMergeWrappedOrganization({
    ...
    const hasWrappedOrganizationShape =
      ...
      (this.looksLikeWrappedEntityContinuationLine(nextLine.text) &&
        this.looksLikeWrappedExperienceEntityHeaderText(combinedText));
  ...
  private static looksLikeWrappedEntityContinuationLine(line: string): boolean {
    const normalizedLine = line.trim();

    return (
      normalizedLine.length >= 2 &&
      normalizedLine.length <= 60 &&
      /^[\p{Ll}(]/u.test(normalizedLine) &&
  ...
  private static previousContentText(
    lines: string[],
    startIndex: number
  ): string | undefined {
    for (let index = startIndex; index >= 0; index--) {
      ...
      if (line !== undefined && !this.isExperienceNoiseLine(line)) {
        return line;
      }
    }
  ...
  private static createExperienceHeaderCandidate(
    ...
    if (
      this.looksLikeWrappedDescriptionContinuationFragment({
        allLines: parserLines,
        index,
  ...
  private static canStartCanonicalExperienceHeaderAt({
    index,
    lineTexts,
    ...
    return (
      this.canStartCanonicalExperienceHeader(line) ||
      this.looksLikeAmbiguousCanonicalOrganizationHeader({
        index,
        line,
        lineTexts,
      })
  ...
  private static looksLikeAmbiguousCanonicalOrganizationHeader({
    ...
    line,
    lineTexts,
  }: CanonicalHeaderCandidateStartParams & { line: string }): boolean {
    const normalizedLine = line.trim();
    ...
    const previousLine = this.previousContentText(lineTexts, index - 1);
    ...
    if (
      hasLocationShape &&
      !hasOrganizationCue &&
      (!previousLine || this.looksLikeDuration(previousLine))
    ) {
      return false;
    }

    return this.looksLikeAmbiguousOrganizationHeaderShape(normalizedLine);
  }

  private static looksLikeAmbiguousOrganizationHeaderShape(
    line: string
  ): boolean {
  ...
  private static canStartCanonicalExperienceHeader(line: string): boolean {
    ...
    const isWrappedExperienceEntity =
      this.looksLikeWrappedExperienceEntityHeaderText(normalizedLine);
  ...
  private static canonicalHeaderLocationLine({
    ...
    return this.looksLikeLocation(text) ||
      this.looksLikeContextualLocationAfterDuration(text) ||
  ...
  private static hasOrganizationHeaderShape(text: string): boolean {
    return (
      ...
      this.looksLikeWrappedExperienceEntityHeaderText(text)
    );
  }
  ...
  private static hasStrongOrganizationCueText(text: string): boolean {
    const normalizedLine = text.trim();
  ...
  private static hasOrganizationDomainCueText(text: string): boolean {
    return /\b(?:AI|Coalition|Connections|Defense|Federation|Forces|Forex|Labs?|Network|Robotics|Services?|Ventures?)\b/u.test(
      text
    );
  }
    ...
    switch (state) {
      ...
      case 'seeking_dates':
        ...
        if (
          ...
          this.looksLikeLocationBeforeDuration(text, index, lineTexts) ||
  ...
  private static looksLikeLowerCamelOrganization(line: string): boolean {
    ...
    if (
      !/^[a-z][\p{L}\p{M}0-9&.'+-]*\p{Lu}/u.test(normalizedLine) ||
      this.looksLikeDuration(normalizedLine) ||
      this.looksLikeLocation(normalizedLine) ||
      this.looksLikePosition(normalizedLine) ||
      this.looksLikeSentenceLikeDescriptionText(normalizedLine)
  ...
  private static looksLikeDescriptionLine({
    ...
    if (/\$(?:[A-Z]{1,8}|\d+(?:\.\d+)?[KMB]?)\b/u.test(normalizedLine)) {
      return true;
    }

    if (this.looksLikeParenthesizedUrlText(normalizedLine)) {
      return true;
    }
  ...
  private static looksLikeDescriptionContinuationLine(
    ...
    if (/^(?:&|\+|and\b|or\b)/iu.test(normalizedLine)) {
      return true;
    }
  ...
  private static looksLikeLocation(line: string): boolean {
    ...
    if (
      /^(?:in|at|for|through)\b/iu.test(normalizedLine) &&
      normalizedLine.split(/\s+/).length > 4
    ) {
      return false;
    }
  ...
  private static looksLikeStandaloneLocationAfterDuration(
    ...
    return (
      ...
      this.looksLikeContextualLocationAfterDuration(line)
    );
  }

  private static looksLikeLocationBeforeDuration(
    line: string,
    index: number,
    allLines: string[]
  ): boolean {
  ...
  private static looksLikeContextualLocationAfterDuration(
    line: string
  ): boolean {
    const normalizedLine = this.normalizeCompletedLocationText(line);
  ...
  private static hasHeaderDetailGeometry({
    line,
    previousLine,
  }: HeaderDetailGeometryParams): boolean {
    const hasAlignedX =
      line.x === undefined ||
  ...
  private static looksLikeShortProperHeaderLocationText(
    ...
    if (
      ...
      /^[-+*•>]/u.test(normalizedLine) ||
      /[$@!?;:]/u.test(normalizedLine) ||
      /\b[\p{L}\p{N}][\p{L}\p{N}-]*\.[\p{L}]{2,}\b/iu.test(normalizedLine) ||
      /[.]$/u.test(normalizedLine) ||
      /https?:\/\//iu.test(normalizedLine) ||
      this.looksLikeDuration(normalizedLine) ||
      this.looksLikePosition(normalizedLine) ||
  ...
  private static looksLikeWrappedDescriptionContinuationFragment({
    ...
    index,
    line,
  }: WrappedDescriptionContinuationFragmentParams): boolean {
    const previousLine = this.previousContentLine(allLines, index - 1);
    const nextLine = this.nextContentLine(allLines, index + 1);
    const normalizedLine = line.text.trim();
    ...
    if (
      ...
      !previousText ||
      /[.!?]$/.test(previousText) ||
      this.looksLikeDuration(previousText) ||
      this.looksLikeLocation(previousText) ||
      normalizedLine.split(/\s+/).length > 3
    ) {
      return false;
    }

    const previousLineContinuesDescription =
      /^[-+*•]/u.test(previousText) ||
      this.DESCRIPTION_CONTINUATION_CONNECTOR_PATTERN.test(previousText);
    const fontContinuesPreviousLine =
      line.fontSize === undefined ||
      previousLine.fontSize === undefined ||
      line.fontSize <= previousLine.fontSize + 0.75;
    const nextLineLooksMoreProminent =
      !nextLine ||
      line.fontSize === undefined ||
   ...
   * Parts are normalized by trimming whitespace and legal-suffix dots first.
   */
        ...
        case 'location':
          if (currentPosition) {
            const locationText =
              ...
              this.normalizeCompletedLocationText(currentPosition.location) !==
                this.normalizeCompletedLocationText(section.text)
  ...
  private static extractCleanOrganizationName(
    ...
    if (this.looksLikeWrappedExperienceEntityHeaderText(text.trim())) {
      return text.trim();
    }
    ...
    if (
      ...
      (this.looksLikeAnchoredOrganizationHeaderText(normalizedText) ||
        this.looksLikeAmbiguousOrganizationHeaderShape(normalizedText))
    ) {
    ...
    return this.looksLikeVisualOrganizationHeaderText(normalizedText) ||
      ...
      this.looksLikeWrappedExperienceEntityHeaderText(normalizedText)
      ? normalizedText
      : undefined;
  }

  private static looksLikeWrappedExperienceEntityHeaderText(
    text: string
  ): boolean {
    ...
    const hasHeaderPunctuation = /[;:&|]/u.test(normalizedText);
    const titleCaseWordCount = words.filter(word =>
      /^[\p{Lu}0-9][\p{L}\p{M}0-9&.'+-]*[.]?$/u.test(word)
    ).length;
    ...
    return (
      words.length >= 3 &&
      words.length <= 24 &&
      hasHeaderPunctuation &&
      titleCaseWordCount >= Math.ceil(words.length * 0.5)
    );
  }
  ...
  private static looksLikeSentenceLikeDescriptionText(line: string): boolean {
    ...
    return (
      ...
      /\b(?:is|are|was|were)\s+(?:a|an|the)\b/iu.test(normalizedLine) ||
  ...
  private static looksLikeShortDescriptorEntryHeader(
    ...
    if (this.looksLikePosition(normalizedLine)) {
      return this.hasOwnDurationBeforeBoundary(index, allLines);
    }

    if (this.looksLikeLoosePositionTitle(normalizedLine, index, allLines)) {
      return true;
    }
